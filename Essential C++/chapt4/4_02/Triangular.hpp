...
class Triangular_iterator //维护一个索引值
{
public:
    Triangular_iterator(int index) : _index(index) {}
    bool operator==(const Triangular_iterator &) const; //运算符重载
    bool operator!=(const Triangular_iterator &) const;
    int operator*() const;
    Triangular_iterator &operator++(); //递增运算符
    Triangular_iterator operator++(int); //递减运算符，带有int
...
class Triangular
{
    friend class Triangular_iterator; //和类建立朋友机制，允许访问私有成员

public:
    typedef Triangular_iterator iterator; //将类名设置别名 iterator 使用
    Triangular(int len = 1, int begin = 1); //默认的构造函数
    
    int length() const { return _length; } //将长度和起始位置通过内链函数设置
    ...
    Triangular &copy(const Triangular &rhs); //复制赋值运算符

    static bool is_elem(int value); //静态变量，独立存在，与类对象无关时使用静态static
...
bool Triangular_iterator::operator!=(const Triangular_iterator &rhs) const
{
    return !(*this == rhs); //this是一个指针，加*代表地址中的值
}
...
// Triangular方法实现
vector<int> Triangular::_elems; //静态成员需要定义
...
bool Triangular::is_elem(int value)
{
    ...
    found_it = find(_elems.begin(), end_it, value); //使用泛型算法
...
ostream &operator<<(ostream &os, const Triangular &rhs) //输出运算符重载
{
